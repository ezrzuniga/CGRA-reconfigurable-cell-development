// CGRA_Final_NoC_Softmax4__TB.cpp
// Puerto directo de cgra_final_TB/CGRA_Final_Softmax4__TB.cpp a
// CGRA_Final_NoC_Mesh: MISMO programa espacial (EXP2 via shift + arbol de
// suma de 2 niveles), MISMOS margenes de ciclos -- igual que los otros 3
// puertos de este directorio (GEMM/SumReduce8/FFT4), la unica diferencia
// real con el original es el include y los typedefs (Mesh/Link/Instr/
// constantes _NOC_).
//
// Cuarta y ultima de las 4 aplicaciones de cgra_final_TB/ portadas a NoC:
// junto con GEMM (bucle repetitivo), SumReduce8 (multiplexado en el tiempo,
// las 4 MAC activas) y FFT4 (reuso de wire con orden de dependencia
// estricto), esta cierra el conjunto con el patron que faltaba -- 4 bordes
// reales SIN multiplexado (a diferencia de SumReduce8) y SIN reuso de wire
// (a diferencia de FFT4), mas una salida que reusa el mismo puerto en 2
// tiempos distintos (SUM en addr10, e0 en addr11) para el spot-check.
//
// Ver cgra_final_TB/CGRA_Final_Softmax4__TB.md para el porque de EXP2 exacto
// via shift (softmax base 2, no base e, sin aproximacion), el arbol de
// reduccion de 2 niveles y la tabla de instrucciones completa -- no se
// repite aca.
...
// Identico al original (cgra_final_TB/CGRA_Final_Softmax4__TB.cpp).
...
static bool run_case(Mesh& mesh, sc_signal<bool>& rst,
    ...
    // motivo que en los otros 3 puertos NoC de este directorio).
...
int sc_main(int argc, char* argv[]) {
    ...
    cout << "\n############################################################\n"
         << "#  Softmax base-2 de 4 elementos sobre CGRA_Final_NoC_Mesh   #\n"
    ...
    cout << "\n############################################################\n";
    if (all_ok) {
        cout << "#  PASS: softmax base-2 correcto en los 3 casos (NoC)       #\n";
    } else {
