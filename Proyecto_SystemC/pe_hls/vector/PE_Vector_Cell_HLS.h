// PE_Vector_Cell_HLS.h
// Igual patron que pe/vector/PE_Vector_Cell.h: los puertos de dato bindean
// directo (Link ya es PE_VectorData<DATA_W,VLEN>, el tipo nativo del core),
// solo instr_in necesita puente (PE_InstrIn de 1 parametro -> PE_VecInstrIn
// de 2 parametros). Sin herencia de base abstracta.
...
class PE_Vector_Cell_HLS : public sc_core::sc_module {
public:
    typedef PE_VectorData<DATA_W, VLEN> Link;
    typedef PE_InstrIn<DATA_W>          InstrIn;
    typedef PE_vector_HLS<DATA_W, VLEN, NUM_REGS, INSTR_MEM_SIZE> Inner;
    ...
    SC_HAS_PROCESS(PE_Vector_Cell_HLS);

    explicit PE_Vector_Cell_HLS(sc_core::sc_module_name name)
        : sc_module(name),
