...
// Port 1:1 de mesh/CGRA_Mesh_2x2_Heterogeneous_Test__TB.cpp sobre
// CGRA_Mesh_Static: mismo layout heterogeneo (Enrutamiento/Memoria/Escalar/
// Vectorial), mismos dos escenarios, mismos valores esperados -- prueba de
// integracion de que el mecanismo de layout estatico + los 4 tipos de celda
// HLS reproducen el comportamiento de la malla runtime original.
//
// Diferencias mecanicas respecto al original (no de comportamiento): sin
// argumento de layout en runtime (el layout esta en la lista de tipos del
// template), y mesh.cell<1>() en vez de static_cast sobre PE_Base.
#include <systemc.h>
#include <sstream>
#include "CGRA_Mesh_Static.h"
...
typedef CGRA_Mesh_Static<ROWS, COLS, 32, 4,
    PE_Routing_Cell_HLS<32, 4>,
    PE_Memory_HLS_Cell<32, 4, 512>,
    PE_Scalar_Cell_HLS<32, 4, 8, 1>,
    PE_Vector_Cell_HLS<32, 4, 8, 1>> Mesh;
...
static Instr routing_instr(sc_uint<4> sel_N, sc_uint<4> sel_S, sc_uint<4> sel_E, sc_uint<4> sel_W) {
    return make_routing_config_instr_hls<32>(sel_N, sel_S, sel_E, sel_W);
}
...
int sc_main(int argc, char* argv[]) {
    ...
    // Acceso tipado directo, sin static_cast sobre una base virtual.
    auto& mem_cell = mesh.cell<1>();  // (0,1)
    ...
    {
        int32_t got = mem_cell.read_sram(0).to_int();
        std::ostringstream in;
        in << "in_W[0](Enrutamiento)=" << in_W[0].read();
    ...
    if (ok) {
        cout << "\nPASS: malla 2x2 heterogenea HLS (Enrutamiento/Memoria/Escalar/Vectorial) "
